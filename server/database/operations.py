...
class HostOperations:
    ...
    def get_hosts_by_status(self, status: str, limit: Optional[int] = None) -> List[Host]:
        ...
        try:
            with self.db_manager.get_session() as session:
                query = session.query(Host).filter(Host.status == status).order_by(desc(Host.last_seen))
    ...
    def mark_hosts_offline_by_timeout(self, timeout_threshold: datetime) -> int:
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                timed_out_hosts = session.query(Host).filter(
                    and_(
                        Host.status == 'online',
                        Host.last_seen < timeout_threshold
                    )
                ).all()
    ...
    def cleanup_old_hosts(self, older_than_days: int) -> int:
        ...
        try:
            ...
            with self.db_manager.get_session() as session:
                ...
                old_hosts = session.query(Host).filter(
                    and_(
                        Host.status == 'offline',
                        Host.last_seen < cutoff_date
                    )
                ).all()
        ...
        try:
            with self.db_manager.get_session() as session:
                hosts = session.query(Host).filter(
                    Host.current_ip.like(ip_pattern)
    ...
    def get_recently_seen_hosts(self, hours: int = 24) -> List[Host]:
        ...
        try:
            ...
            with self.db_manager.get_session() as session:
                hosts = session.query(Host).filter(
                    Host.last_seen >= since_time
                ).order_by(desc(Host.last_seen)).all()
    ...
    def get_host_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                online_hosts = session.query(func.count(Host.id)).filter(Host.status == 'online').scalar() or 0
                offline_hosts = session.query(func.count(Host.id)).filter(Host.status == 'offline').scalar() or 0
                
                # Recent activity (last 24 hours)
                since_24h = datetime.now(timezone.utc) - timedelta(hours=24)
                recent_hosts = session.query(func.count(Host.id)).filter(
                    Host.last_seen >= since_24h
                ).scalar() or 0
