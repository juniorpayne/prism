...
class TCPServerConfig:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"TCP server configured: {self.host}:{self.tcp_port}, "
...
class TCPServer:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            logger.info(f"TCP server started successfully on {self.config.host}:{self.config.tcp_port}")
    ...
    async def _graceful_shutdown(self) -> None:
        ...
        try:
            ...
            while (self.connection_manager.get_active_count() > 0 and
                   time.time() - shutdown_start < self.config.graceful_shutdown_timeout):
                
                logger.info(f"Waiting for {self.connection_manager.get_active_count()} connections to close")
                await asyncio.sleep(1.0)
            
            # Force close any remaining connections
            if self.connection_manager.get_active_count() > 0:
                logger.warning(f"Force closing {self.connection_manager.get_active_count()} remaining connections")
                await self.connection_manager.close_all_connections()
    ...
    async def _handle_client_connection(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            ...
            if not await self.connection_manager.add_connection(connection_handler):
                logger.warning(f"Connection limit reached, rejecting {connection_handler.client_ip}")
