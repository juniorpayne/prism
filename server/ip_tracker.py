...
class IPTrackerConfig:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"IP tracker configured: logging={self.enable_change_logging}, "
...
class IPTracker:
    ...
    async def detect_ip_change(
        self, 
        hostname: str, 
        new_ip: str
    ) -> Optional[IPChangeDetection]:
        """
    ...
    async def log_ip_change(
        ...
        try:
            ...
            logger.info(f"IP change logged: {hostname} {previous_ip} -> {new_ip} "
    ...
    async def get_ip_change_history(
        ...
        try:
            ...
            host_history = [
                event for event in self._change_history
    ...
    async def get_ip_change_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            changes_last_hour = len([
                event for event in self._change_history
                if event.change_time > hour_ago
            ])
            
            changes_last_day = len([
                event for event in self._change_history
                if event.change_time > day_ago
            ])
    ...
    async def cleanup_old_ip_changes(self, older_than_days: int = 90) -> int:
        ...
        try:
            ...
            old_entries = [
                event for event in self._change_history
                if event.change_time < cutoff_time
            ]
            ...
            if removed_count > 0:
                logger.info(f"Cleaned up {removed_count} old IP change records "
    ...
    async def _update_ip_type_stats(self, previous_ip: str, new_ip: str) -> None:
        ...
        try:
            ...
            try:
                ...
                if isinstance(prev_ip, ipaddress.IPv4Address) or isinstance(new_ip_obj, ipaddress.IPv4Address):
                    self._stats['ipv4_changes'] += 1
                
                if isinstance(prev_ip, ipaddress.IPv6Address) or isinstance(new_ip_obj, ipaddress.IPv6Address):
    ...
    async def get_hosts_with_ip_changes(
        self, 
