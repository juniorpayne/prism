...
class HostManagerConfig:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"Host manager configured: status_check={self.status_check_interval}s, "
...
class HostManager:
        ...
        if existing_host.status == 'offline':
            ...
            if existing_host.current_ip != ip_address:
                ...
                if success:
                    ...
                    logger.info(f"Host reactivated with IP change: {hostname} "
    ...
    async def cleanup_old_hosts(self, older_than_days: Optional[int] = None) -> int:
        ...
        try:
            ...
            if cleaned_count > 0:
                logger.info(f"Cleaned up {cleaned_count} old hosts (offline > {older_than_days} days)")
    ...
    async def get_hosts_last_seen_before(
        self, 
