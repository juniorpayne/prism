...
class ServerApplication:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            logger.info(f"TCP server listening on {self.config.server.host}:{self.config.server.tcp_port}")
            logger.info(f"API server listening on {self.config.server.host}:{self.config.server.api_port}")
    ...
    async def _start_tcp_server(self) -> None:
        ...
        try:
            ...
            await self.tcp_server.start(
                ...
                port=self.config.server.tcp_port
            )
            logger.info(f"TCP server started on {self.config.server.host}:{self.config.server.tcp_port}")
    ...
    async def _start_api_server(self) -> None:
        ...
        try:
            ...
            logger.info(f"API server started on {self.config.server.host}:{self.config.server.api_port}")
...
Environment Variables:
  ...
  PRISM_LOGGING_LEVEL       - Override logging level
    ...
    parser.add_argument(
        '--version', '-v',
        action='version',
        version='Prism DNS Server 1.0.0'
    )
