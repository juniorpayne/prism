...
class HeartbeatConfig:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"Heartbeat monitor configured: check_interval={self.check_interval}s, "
...
class HeartbeatMonitor:
    ...
    def calculate_timeout_threshold(
        ...
        logger.debug(f"Calculated timeout threshold: {threshold}s "
    ...
    async def check_host_timeouts(
        ...
        self._statistics['average_check_duration'] = sum(self._statistics['check_durations']) / len(self._statistics['check_durations'])
        ...
        logger.info(f"Heartbeat check completed: {result.hosts_checked} hosts checked, "
    ...
    async def mark_hosts_offline(
        ...
        logger.info(f"Status change operation completed: {hosts_marked_offline}/{hosts_processed} hosts marked offline "
                   f"in {operation_duration:.3f}s")
        
        return result
    
    async def get_all_online_hosts(
        self,
        limit: Optional[int] = None
    ) -> List[Host]:
        """
    ...
    async def start_monitoring(self, heartbeat_interval: int = 60) -> None:
        ...
        """
        logger.info(f"Starting heartbeat monitoring loop (check every {self.config.check_interval}s)")
            ...
            try:
                ...
                if timeout_result.timed_out_hosts:
                    status_result = await self.mark_hosts_offline(
                        timeout_result.timed_out_hosts,
                        "heartbeat_timeout"
                    )
                    
                    if not status_result.success:
                        logger.warning(f"Some hosts failed to be marked offline: {status_result.failed_hosts}")
    ...
    async def cleanup_old_offline_hosts(self) -> int:
        ...
        """
        try:
            cutoff_time = datetime.now(timezone.utc) - timedelta(days=self.config.cleanup_offline_after_days)
            ...
            with host_ops.db_manager.get_session() as session:
                ...
                old_offline_hosts = session.query(Host).filter(
                    Host.status == 'offline',
