...
class AdvancedHostnameValidator:
    ...
    def __init__(self):
        ...
        self.reserved_hostnames = {
            'localhost', 'broadcasthost', 'local', 'localdomain',
            'example', 'test', 'invalid', 'onion', 'exit'
        }
    ...
    def validate_hostname_comprehensive(self, hostname: str) -> Tuple[bool, Optional[str], Dict[str, Any]]:
        """
        ...
        for label in labels:
            ...
            if label.startswith('-') or label.endswith('-'):
                return False, f"Label cannot start or end with hyphen: '{label}'", validation_details
        ...
        for pattern in self.typosquatting_patterns:
            if pattern.search(hostname_lower):
                validation_details['warnings'].append(f"Potential typosquatting detected: {hostname}")
...
class RegistrationMessageValidator:
    ...
    def validate_registration_message(
        ...
        hostname_valid, hostname_error, hostname_details = self.hostname_validator.validate_hostname_comprehensive(hostname)
