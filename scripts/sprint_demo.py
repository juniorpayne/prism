...
class SprintDemo:
    ...
    async def demo_scrum_18_configuration(self):
        ...
        self.print_step(
            "2. Environment Variable Overrides", "Testing PRISM_* environment variables"
        )
    ...
    async def demo_scrum_17_api(self):
        ...
        self.print_success(
            f"Pagination logic: Page 1 of {len(all_hosts)} hosts (page_size={page_size})"
        )
    ...
    async def demo_scrum_12_docker(self):
        ...
        self.print_success(
            f"Docker environment files present: {len(existing_files)}/{len(docker_files)}"
        )
