...
def demo_basic_logging():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        log_manager.log_debug("Debug message for troubleshooting", component="DemoComponent")
        log_manager.log_info(
            "Application started successfully", component="DemoComponent", version="1.0.0"
        )
...
def demo_log_levels_filtering():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        config = {"logging": {"level": "WARNING", "file": log_file, "console": False}}
...
def demo_error_handling():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        config = {"logging": {"level": "INFO", "file": log_file, "console": True}}
...
def demo_log_rotation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i in range(30):  # Should exceed 2KB
            log_manager.log_info(
                f"Large message {i}: {large_message}", component="RotationDemo", iteration=i
            )
...
"""
    ...
    try:
        ...
        # Test logging
        log_manager.log_info(
            "Configuration-based logging test", component="ConfigDemo", config_file=config_file
        )
...
def demo_real_world_scenario():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Client startup
        log_manager.log_info(
            "Prism client starting up", component="Main", version="1.0.0", config_file="client.yaml"
        )

        # System info gathering
        log_manager.log_info("Gathering system information", component="SystemInfo")

        hostname = "production-client-001"
        log_manager.log_info("Hostname detected", component="SystemInfo", hostname=hostname)
        ...
        log_manager.log_info("Starting heartbeat loop", component="HeartbeatManager", interval=5)
        ...
        # Client shutdown
        log_manager.log_info("Graceful shutdown initiated", component="Main", reason="user_request")

        log_manager.log_info("All components stopped", component="Main", uptime_seconds=300)
