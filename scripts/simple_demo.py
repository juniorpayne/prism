...
def main():
    ...
    print_success(
        f"Configuration loaded: TCP={config.server.tcp_port}, API={config.server.api_port}"
    )
    ...
    monitor_config = {
        ...
        "heartbeat": {"check_interval": 30, "timeout_multiplier": 2, "grace_period": 30},
    }
    ...
    print_success(
        f"Overall: {passed_tests}/{total_tests} tests passing ({(passed_tests/total_tests)*100:.1f}%)"
    )
