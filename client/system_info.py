...
class SystemInfo:
    ...
    def validate_hostname(self, hostname: str) -> None:
        ...
        for label in labels:
            ...
            if len(label) > 63:
                raise HostnameValidationError(f"Hostname label too long: '{label}' (max 63 characters)")
            ...
            if label.startswith('-') or label.endswith('-'):
                raise HostnameValidationError(f"Hostname label cannot start or end with hyphen: '{label}'")
