...
class TestUserProfile:
    ...
    async def test_get_profile_success(self, async_client: AsyncClient, authenticated_user):
    ...
    async def test_update_profile_success(
        ...
        response = await async_client.put(
            "/api/users/me", json=update_data, headers={"Authorization": f"Bearer {token}"}
        )
        ...
        result = await db_session.execute(
            select(UserActivity).where(
                UserActivity.user_id == user.id, UserActivity.activity_type == "profile_updated"
            )
        )
    ...
    async def test_change_password_success(
        ...
        result = await db_session.execute(
            select(UserActivity).where(
                UserActivity.user_id == user.id, UserActivity.activity_type == "password_changed"
            )
        )
        activity = result.scalar_one_or_none()
        assert activity is not None
    ...
    async def test_delete_account_success(
        ...
        response = await async_client.delete(
            "/api/users/me",
            json={"password": "TestPassword123!", "confirmation": "DELETE MY ACCOUNT"},
            headers={"Authorization": f"Bearer {token}"},
        )
        ...
        result = await db_session.execute(
            select(UserActivity).where(
                UserActivity.user_id == user.id, UserActivity.activity_type == "account_deleted"
            )
        )
        activity = result.scalar_one_or_none()
        assert activity is not None
    ...
    async def test_delete_account_wrong_password(
        ...
        response = await async_client.delete(
            "/api/users/me",
            json={"password": "WrongPassword123!", "confirmation": "DELETE MY ACCOUNT"},
            headers={"Authorization": f"Bearer {token}"},
        )
    ...
    async def test_get_activity_log(
        ...
        response = await async_client.get(
            "/api/users/me/activity", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    async def test_get_activity_log_pagination(
        ...
        # Get first page
        response = await async_client.get(
            "/api/users/me/activity?page=1&limit=10", headers={"Authorization": f"Bearer {token}"}
        )
        ...
        # Get second page
        response = await async_client.get(
            "/api/users/me/activity?page=2&limit=10", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    async def test_get_settings(self, async_client: AsyncClient, authenticated_user):
        """Test getting user settings."""
        user, token = authenticated_user

        response = await async_client.get(
            "/api/users/me/settings", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    async def test_update_settings(
        ...
        response = await async_client.put(
            ...
            json={"theme": "light", "email_notifications": False, "session_timeout": 60},
            headers={"Authorization": f"Bearer {token}"},
        )
        ...
        result = await db_session.execute(
            select(UserActivity).where(
                UserActivity.user_id == user.id, UserActivity.activity_type == "settings_updated"
            )
        )
        activity = result.scalar_one_or_none()
        assert activity is not None
    ...
    async def test_token_invalidation_after_password_change(
        ...
        result = await db_session.execute(
            select(TokenBlacklist).where(TokenBlacklist.token == "test_refresh_token")
        )
