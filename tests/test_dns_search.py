...
def test_search_zones(client):
    ...
    mock_client.search_zones.assert_called_once_with(
        query="example", zone_type=None, hierarchy_level=None, limit=100
    )
...
def test_search_zones_with_filters(client):
    ...
    with patch("server.api.routes.dns.get_powerdns_client") as mock_get_client:
        ...
        response = client.get("/api/dns/zones/search?q=example&zone_type=Master&hierarchy_level=0")
    ...
    mock_client.search_zones.assert_called_once_with(
        query="example", zone_type="Master", hierarchy_level=0, limit=100
    )
...
def test_search_records_by_content(client):
    ...
    mock_client.search_records.assert_called_once_with(
        query="192.168", record_type=None, zone_name=None, content_search=True, limit=100
    )
...
def test_search_records_in_zone(client):
    ...
    mock_client.search_records.assert_called_once_with(
        query="www", record_type=None, zone_name="test.com.", content_search=False, limit=100
    )
...
def test_filter_zones(client):
    ...
    with patch("server.api.routes.dns.get_powerdns_client") as mock_get_client:
        ...
        filters = {"min_records": 5, "max_records": 15, "has_dnssec": False}
...
def test_filter_zones_with_sorting(client):
    ...
    with patch("server.api.routes.dns.get_powerdns_client") as mock_get_client:
        ...
        response = client.post("/api/dns/zones/filter?sort_by=records&sort_order=desc", json={})
    ...
    mock_client.filter_zones.assert_called_once_with(
        filters={}, sort_by="records", sort_order="desc"
    )
