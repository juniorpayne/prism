...
class TestDNSIntegration:
    ...
    @pytest.mark.asyncio
    async def test_host_registration_creates_dns_record(
        self, integration_config, db_manager, dns_client, host_ops
    ):
    ...
    async def test_bulk_registration_performance(self, dns_client, host_ops):
        ...
        print(
            f"Bulk registration: {num_hosts} hosts in {duration:.2f}s ({num_hosts/duration:.1f} hosts/sec)"
        )
    ...
    async def test_ttl_configuration(self, dns_client):
        ...
        result = await dns_client.create_a_record(hostname, "10.0.8.1", ttl=custom_ttl)
        assert result["status"] == "success"
    ...
    async def test_zone_validation(self, dns_client):
        ...
        try:
            await dns_client.create_a_record(hostname, "10.0.9.1", zone="nonexistent.zone.")
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(
        not os.getenv("RUN_PERFORMANCE_TESTS"), reason="Performance tests disabled by default"
    )
